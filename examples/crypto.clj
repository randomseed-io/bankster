...
(comment
  ...
  (api-ops/* one-wei 1000000000000000000M)
  ;; => #money/crypto[1 ETH]

  (api-ops/* one-satoshi 100000000M)
  ;; => #money/crypto[1 BTC]
...
(defn portfolio-value
  ...
  (let [values (for [[currency balance] portfolio
                 ...
                 (api-ops/* (api-money/of target-currency rate)
                            (api-money/amount balance)))]
    (apply api-ops/+ values)))
...
(defn simulate-dca
  ...
  (let [purchases (for [rate historical-rates]
    ...
    {:purchases      purchases
     :total-spent    (api-ops/* monthly-amount (count historical-rates))
     :total-btc      (apply api-ops/+ (map :btc purchases))
...
(defn find-arbitrage
  ...
  (let [sorted     (sort-by :price exchange-prices)
        ...
        spread     (api-ops/- (:price expensive) (:price cheapest))
...
(defn calculate-staking-rewards
  ...
  (let [daily-rate (/ apy 365M)
        ...
        final-val  (api-ops/* staked-amount (bigdec multiplier))]
    {:staked      staked-amount
     :apy         (str (* 100 apy) "%")
     :days        days
     :final-value final-val
     :reward      (api-ops/- final-val staked-amount)}))
