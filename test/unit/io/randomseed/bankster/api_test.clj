    ...
    (with-default-currency
      ...
      (fn []
        ...
        (testing "currency accessors and predicates"
          ...
          (is (true? (api/currency? (api/currency :EUR r))))
          (is (false? (api/currency? :EUR)))
      ...
      (fn []
        ...
        (testing "money accessors"
          (let [m (api/money 1.2300M :EUR r)]
            ...
            (is (= 2 (api/money-scale (api/currency :EUR r))))
            (is (nil? (api/money-scale (api/currency :XAU r))))
        ...
        (testing "money predicates"
          (let [m0 (api/money 0 :EUR r)
            ...
            (is (true? (api/money-is-zero? m0)))
            (is (false? (api/money-is-zero? m1)))
    ...
    (with-default-currency
      ...
      (fn []
        (let [m1   (api/money 1 :EUR r)
          ...
          (is (instance? Money (api/+ m1 m2)))
          (is (= 3 (api/+ 1 2)))
          (is (= 1 (api/- 3 2)))
          (is (= 6 (api/* 2 3)))
          (is (= 2 (api// 4 2)))
          (is (true? (api/= m1 (api/money 1 :EUR r))))
          (is (true? (api/not= 1 2)))
          (is (true? (api/> m2 m1)))
          (is (true? (api/>= 2 2)))
          (is (true? (api/< 1 2)))
          (is (true? (api/<= 2 2)))
          (is (neg? (api/compare m1 m2)))
          (is (true? (api/pos? m1)))
          (is (true? (api/neg? mneg)))
          (is (== 1 (api/long m1)))
          (is (== 1 (api/int m1)))
          (is (== 1.0 (api/double m1)))
          (is (== 1.0 (double (api/float m1)))))))))
